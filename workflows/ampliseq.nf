...
workflow AMPLISEQ {
    ...
    if ( params.input ) {
        ...
        ch_input_reads = channel.fromList(samplesheetToList(params.input, "${projectDir}/assets/schema_input.json")) // meta: meta.sample, meta.run
                       .map{ meta, fastq_1, fastq_2, forwardReads, reverseReads ->
                def readfw = fastq_1 ?: forwardReads
                def readrv = fastq_2 ?: reverseReads
                meta.single_end = single_end.toBoolean()
                def reads = single_end ? readfw : [readfw,readrv]
                if ( !meta.single_end && !readrv ) { error("Entry `fastq_2` (or legacy `reverseReads`) is missing in $params.input for $meta.sample, either correct the samplesheet or use `--single_end`, `--pacbio`, or `--iontorrent`") } // make sure that reverse reads are present when single_end isn't specified
                if ( !meta.single_end && ( readfw.getSimpleName() == meta.sample || readrv.getSimpleName() == meta.sample ) ) { error("Entry `sample` (or legacy `sampleID`) cannot be identical to simple name of `fastq_1`/`fastq_2` (or legacy `forwardReads`/`reverseReads`), please change `sample` in $params.input for sample $meta.sample") } // sample name and any file name without extensions aren't identical, because rename_raw_data_files.nf would forward 3 files (2 renamed +1 input) instead of 2 in that case
                if ( meta.single_end && ( readfw.getSimpleName() == meta.sample+"_1" || readfw.getSimpleName() == meta.sample+"_2" ) ) { error("Entry `sample` (or legacy `sampleID`) + `_1` or `_2` cannot be identical to simple name of `fastq_1` (or legacy `forwardReads`), please change `sample` in $params.input for sample $meta.sample") } // sample name and file name without extensions aren't identical, because rename_raw_data_files.nf would forward 2 files (1 renamed +1 input) instead of 1 in that case
                return [meta, reads] }
