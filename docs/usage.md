...
- Must contain the headers `sample` and `fastq_1`
- May contain the headers `fastq_2`, `run`, `control`, and `quant_reading`
- Legacy headers `sampleID`, `forwardReads`, and `reverseReads` are also supported for backward compatibility
